...
import (
	...
	"github.com/zmloong/lotus/lib/modules/live/amf"
...
func (connClient *ConnClient) Start(url string, method string) error {
	...
	log.Debugf("connection:", "local:", conn.LocalAddr(), "remote:", conn.RemoteAddr())
	...
	if err := connClient.writeCreateStreamMsg(); err != nil {
		log.Debugf("writeCreateStreamMsg error", err)
		return err
	}

	log.Debugf("method control:", method, av.PUBLISH, av.PLAY)
