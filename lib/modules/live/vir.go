...
import (
	...
	"github.com/zmloong/lotus/lib/modules/live/container/flv"
	"github.com/zmloong/lotus/lib/modules/live/liveconn"
	"github.com/zmloong/lotus/lib/modules/live/utils/uid"
...
type (
	...
	StreamReadWriteCloser interface {
		GetInFo
		Close(error)
		Write(liveconn.ChunkStream) error
		Read(c *liveconn.ChunkStream) error
	}
	StaticsBW struct {
		StreamId               uint32
...
func (v *VirReader) Read(p *av.Packet) (err error) {
	defer lotus.Recover("live VirReader Read")
...
func (v *VirReader) Close(err error) {
	log.Debugf("publisher ", v.Info(), "closed: "+err.Error())
	v.conn.Close(err)
}
...
func (v *VirWriter) Close(err error) {
	log.Warnf("player ", v.Info(), "closed: "+err.Error())
	if !v.closed {
		close(v.packetQueue)
	}
	v.closed = true
