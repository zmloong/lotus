...
import (
	...
	"github.com/zmloong/lotus/lib/modules/live/cache"
	"github.com/zmloong/lotus/lib/modules/live/rtmprelay"
...
//
// RtmpStream----------------------------------------------------------------------------------------------------------------------------------------------------------
func NewRtmpStream(options IOptions) *RtmpStream {
	ret := &RtmpStream{
		streams: &sync.Map{},
		options: options,
	}
...
func (rs *RtmpStream) HandleReader(r av.ReadCloser) {
	...
	if stream, ok = i.(*Stream); ok {
		...
		if id != EmptyID && id != info.UID {
			ns := NewStream(rs.options)
			stream.Copy(ns)
			stream = ns
			rs.streams.Store(info.Key, ns)
		}
	} else {
		stream = NewStream(rs.options)
...
func (s *Stream) StartStaticPush() {
	...
	pushurllist, err := rtmprelay.GetStaticPushList(s.options.GetStaticPushUrlList(appname))
